...
export default function Bot(url, deviceId, autoStart, clientCallback, fromKotlin = true, ttsFileType = 'mp3') {
	...
	bot.getFiles = async (query, index) => {
		const data = {query: query, index: index}
		return await axios.post("https://manual-search-develop.alquist.ai/retrieve", data, {"Content-Type": "application/json"});
	}
