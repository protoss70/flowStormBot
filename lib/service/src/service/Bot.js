...
export default function Bot(url, deviceId, autoStart, clientCallback, fromKotlin = true, ttsFileType = 'mp3') {
	...
	bot.getFiles = async (query, names, ids, top_k=10) => {
		const data = {query: query, names, ids, top_k}
		var results = null;
		try{
			results = await axios.post("https://manual-search-develop.alquist.ai/retrieve", data, {"Content-Type": "application/json"});
		}
		catch{
			results = {}
		}
	...
	bot.addRecord = function() {
		if (isNotNil(receivedRecords) && receivedRecords.length > 0) {
			...
			if (service.sessionId && sleepTimeLimit === 0) {
			    ...
			    setTimeout(() => { handleAudioInput(!userTyping); }, delay)
			} else {
	...
	function handleAudioInput(start) {
        service.sendLogs(turnLogs);
        turnLogs = [];
		console.log("inp", inputAudio, "start: ", start, "audioOpen", audioOpen, "sleepLim",sleepTimeLimit < getCurrentMillis());
		console.log(start && !audioOpen && sleepTimeLimit < getCurrentMillis());
		if (inputAudio) {
			if (start && true && sleepTimeLimit < getCurrentMillis()) {
				playSound('listening');
				service
					.getStt()
					.start()
	...
	function skipPlayedMessages() {
		...
		if (currentAudio.src !== '') {
			...
			if (isNotNil(receivedRecords) && receivedRecords.length > 0) {
				console.log("recieved records: ", receivedRecords);
