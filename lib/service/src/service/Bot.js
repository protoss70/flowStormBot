...
export default function Bot(url, deviceId, autoStart, clientCallback, fromKotlin = true, ttsFileType = 'mp3') {
	...
	bot.signIn = async () => {
		const user = await fire.SignIn()
		if (user){
			sessionStorage.setItem("flowstorm-bot-user", fire.currentUser().uid);
		}
		return user; 
	}
	...
	bot.updateUser = async (updates) => {
		const userUID = fire.currentUser().uid;
		fire.update(userUID, updates)
	} 
	...
	bot.getUser = async () => {
		...
		var uid = null;
		if (user === null){
			uid = sessionStorage.getItem("flowstorm-bot-user");
		}else{
			uid = user._delegate.uid;
		}
		
		if (uid){
			const res = await fire.findUser(uid);
			if (res._delegate._document === null){
				return null;
			}else{
				return res._delegate._document.data.value.mapValue.fields;
			}
		}
		return null
	}
