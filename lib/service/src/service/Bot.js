...
export default function Bot(
  ...
  const audios = {
    listening: new Audio(
      "https://repository.flowstorm.ai/audio/client/listening.mp3"
    ),
    recognized: new Audio(
      "https://repository.flowstorm.ai/audio/client/recognized.mp3"
    ),
    waiting: new Audio(
      "https://repository.flowstorm.ai/audio/client/waiting.mp3"
    ),
  ...
  bot.playAudio = (audio) => {
    ...
    if (audio && outputAudio) {
      ...
      currentAudio.play().catch((error) => {
        errorCallback({
          type: "Client",
          message: `Audio error in ${currentAudio.src}:  ${error}`,
        });
      });
    }
  ...
  bot.getFiles = async (
    query,
    url = "https://manual-search-develop.alquist.ai/retrieve"
  ) => {
    const data = { query };
    var results = null;
    try {
  ...
  bot.sendVote = (turnId, nodeId, vote) => {
    service.sendVote(turnId, nodeId, vote);
  }
  ...
  function initialize() {
    ...
    currentAudio = new Audio(
      "https://repository.flowstorm.ai/audio/client/intro.mp3"
    );
    ...
    service
      ...
      .catch((error) => {
        errorCallback({
          type: "Client",
          message: `Error opening socket:  ${error}`,
        });
      });
    return this;
  }
  ...
  bot.addRecord = function () {
    if (isNotNil(receivedRecords) && receivedRecords.length > 0) {
      ...
      if (clientCallback.focusOnNode && nodeId !== 0)
        clientCallback.focusOnNode(nodeId);
      if (video) {
        ...
        if (
          ...
          if (text !== "#walk") bot.addRecord();
        } else {
          clientCallback.addMessage(
            "received",
            text,
            image,
            background,
  ...
  const itemMap = ({
    ...
    background: isNotNil(background)
      ? background.length === 0
        ? null
        : background
      : null,
  ...
  function onMessage(param) {
    ...
    switch (param.type) {
      case "ResponseItem":
        ...
        if (
          param.responseItem.ttsConfig &&
          param.responseItem.ttsConfig.locale &&
        ...
        break;
      case "Response":
        bot.responses.push(paramResponse);
        setClientStatus("RESPONDING");
        // TODO remove
        stopWaitSound();
        ...
        if (paramResponse.sleepTimeout > 0) {
          sleepTimeLimit =
            getCurrentMillis() + paramResponse.sleepTimeout * 1000;
        }
      ...
      case "Recognized":
        ...
        const recognizedItems =
          param.message === undefined ? [param] : param.message.items;
        const recognizedItem = recognizedItems[0];
        // const bulkMessages = transformIncomingMessages(recognizedItems);
        if (
          recognizedItem.text.length >= lastSttResult.length ||
          recognizedItem.isFinal
        ) {
    ...
    if (audioOpen) {
      service
        ...
        .catch(() => {
          errorCallback({
            type: "Client",
            message: `Speech to text stop error in ${origin}`,
          });
        });
    }
  };
        ...
        service
          ...
          .catch((e) => {
            if (saveSession) {
              ...
              errorCallback({
                type: "Client",
                message: `Speech to text start error: ${e}`,
              });
            }
          });
      } else {
  ...
  bot.handleOnTextInput = (text, audioOn, show = true) => {
  ...
  function addSentMessage(messageText) {
    const text =
      messageText.charAt(0) === "#" && maskSignals ? null : messageText;
