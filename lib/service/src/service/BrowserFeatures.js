export default class BrowserFeatures {
	...
	getSpeechRecognition = async () => {
		...
		if (!navigator.userAgent.includes('Chrome') && (navigator.userAgent.includes('Edg') || navigator.userAgent.includes('Edge'))) {
			return Promise.reject('Web Speech API.SpeechRecognition does not work in Edge');
		}
		...
		console.log(mobile, chromeMobile);

		if (this.state.SpeechRecognition) {
			return mobile && !chromeMobile ? Promise.resolve(false) : Promise.resolve(this.state.SpeechRecognition);
		}
